...
| `interaction_cancelled` | `execution` | The operator cancelled a pending interaction |
...
`run`, `watch`, `action run`, and the migration shortcut accept `--headless fail|wait` and `--timeout`. Headless defaults to `fail`: a declared prompt stops the task with `interaction_required`, closes the diagnostic prompt, and cleans up owned resources. Supply known action inputs first. Explicit `--headless wait` leaves a discoverable waiting operation; each prompt expires after five minutes or the earlier operation/context deadline. A nonzero operation timeout must be at least 1 ms and covers execution, including daemon work after an observer disconnect. It is distinct from the existing `flush --timeout` wait budget. No headless mode automatically confirms choices. A service’s readiness budget pauses during that attempt’s operator wait; the operation and prompt deadlines keep running.
...
Use another CLI while the first command waits. Prompt metadata contains `id`, `runId`, `task`, `attemptId`, `kind`, `message`, optional `secret` and ordered `choices`, `state`, `createdAt` and `deadline`. States are `pending`, `answered`, `cancelled` and `expired`. Inspection reconstructs deadline/cancellation state without changing retained prompt metadata. Responses require exactly one boolean `--confirm true|false`, text `--text`, text `--stdin`, zero-based `--choice <index>`, or `--cancel`. False, empty text and choice zero are valid answers. Selection indexes must address the persisted `choices` list; cancel stops the interaction with `interaction_cancelled`. Prompt text plus choices is bounded to 64 KiB and at most 1000 choices. Events carry the same options in `promptChoices`. Compact views omit the entire choices list if it cannot fit; retrieve full prompt metadata before answering. Stdin reads UTF-8 through EOF, removes one trailing LF or CRLF, and is limited to 64 KiB. Kind mismatches return `invalid_prompt_answer`; identity mismatches return `prompt_mismatch`; duplicate, cancelled, expired or finished requests return `prompt_not_pending`. Prompt identity is unique across parallel tasks and retries. Only the latest active attempt of a task may create, answer or consume a prompt; a stopped or replaced service attempt is closed even while its old process reader is still unwinding. An answer accepted just before that transition is removed without delivery. Response admission shares a cross-process lock with run cancellation, finalization and pruning.
...
On a dirty-worktree preflight failure, stderr progress lists the sorted changed paths, quoted to keep spaces and control characters unambiguous. This uses the bounded `repositoryChanges.changedPaths` sample before output-detail filtering, so paths remain visible with `--details issues` and in GitHub Actions console output. If the sample omits paths, progress reports the displayed/total counts and directs the user to `git status --short --untracked-files=all`. `--progress quiet` suppresses these progress lines as usual.
...
TUI diagnostics separate decoded Escape/Ctrl+C observations from handler decisions, including modal consumption and forwarding to tview. They retain modifiers and focus/modal context with unknown input origin and intent, excluding printable typing and prompt answers; `q` is logged only when its quit binding executes. Application-loop return precedes owned cleanup, and `status=returned` describes return rather than user intent. The first failure remains authoritative when later cleanup also fails.

For bare/TUI launches on Windows, the bootstrap parent appends child start, cancellation/interruption/termination and actual native numeric/hex exit status to `tui.log`. TUI-owned stop requests carry an ID also recorded with the caller PID and cleanup outcome in `daemon.log`; a shutdown message alone does not establish a task failure. These append-only logs use existing JSONL retrieval, without a separate report command or session index. Native input records and decoder internals unavailable through public library hooks remain explicitly unknown.
...
Task logs are created once at `runs/<run-id>/attempts/<attempt-id>.log` beneath the instance state directory, then appended only within that attempt. A new attempt never truncates an older log. `logs <task>` selects the current status attempt, taking its pathname and run/attempt identity from the same status snapshot; `--run` selects the latest retained attempt of that task within the chosen run, and `--attempt` selects one exact attempt and requires `--run`. The task and attempt must match. JSON task log records include `task`, `runId`, `attemptId` and `line`. Retained task text uses plain stdout and a leading `E: ` for stderr; `line` and paged log `text` return those bytes without decoding or coloring. Leading `E: ` is reserved by this compact text format. Live events still carry separate `stream` and original `line` fields. Selection is pinned when the command starts: `--follow` follows that one attempt, including when selected from current status. Reissue the command after a watch rerun or restart to select its new attempt. Task, daemon, TUI, and event-stream logs are owner-only (`0600`) on Unix-like systems.
...
- task-log lines and failure excerpts show stdout in the normal text color and stderr in red; the TUI hides the retained `E: ` error marker
...
- `r` immediately invalidates and reruns the selected task scope without a confirmation modal; the daemon still calculates and applies the scoped lifecycle action
...
- normal responsive layouts place the task selector left of the log workspace at 120+ columns with at least 24 rows, retain the stacked compact layout below that breakpoint, preserve a selectable task row before hiding the optional log pane, and show a deliberate too-small fallback below 40x10. Attention mode keeps both a selectable task row and logs visible at every supported size
