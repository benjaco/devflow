...
TUI diagnostics record exit keys before cleanup as `event=tui_exit_requested reason=key_escape|key_q|key_ctrl_c`. These identify received input events, not proof of a physical keypress. Consumed modal keys do not count. The final `tui_stopped` line includes `status` and `reason`: the exit-key reason, `panic`, `error`, or `application_returned` when no explicit exit request/error was observed. `status=ok` alone does not establish that an exit was intentional. When the TUI owns the daemon, its deferred cleanup requests stop-all after the UI exits; `daemon shutdown requested` is therefore not evidence that a task crash initiated daemon shutdown.
