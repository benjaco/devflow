...
TUI tests render simulation screens at 60x24, 80x24, 100x12, 100x30, and 140x40 and cover help/modal key routing, focus, persistent action errors, stable ordering, every lifecycle badge, bounded log tails, pause/resume/task-switch following, and detached quit guidance. The first-draw regression must call `tview.Application.Run` on a simulation screen, wait with a bounded timeout, exercise navigation/help/modal/quit, and assert screen finalization; calling layout helpers directly is not lock-faithful. Keep lifecycle execution tests below the TUI layer as well; a presentation test alone cannot prove that an unrelated service survives.
