...
- CLI JSON output shape, including command-level lifecycle coverage for `run`, lossless burst CI stderr progress with stdout-only final failure diagnostics and bounded early-marker excerpts, `status`, `logs`, `instances`, target-scoped required-env doctor/strict exits, cache key/path/manifest handoff, and `stop`
...
- sequential engine execution with cache hits/misses, per-node duration/cache timings, bounded failure log tails/excerpts, planned target-cache keys that match execution keys, and cache-key manifest creation/reuse/rejection including local/generated-input changes and one total semantic-callback invocation
