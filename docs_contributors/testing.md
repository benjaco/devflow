...
For PowerShell commands, Windows prerequisites, native Payload/TUI evidence and
the disposition of platform skips, see [Native Windows verification](windows-verification.md).
Both E2E opt-ins must be explicit: `DEVFLOW_E2E_PAYLOAD=1` and
`DEVFLOW_E2E_DOCKER=1`. An explicitly requested Docker run fails on unavailable
Engine/client prerequisites; `-short` still excludes the Docker integration layer.
...
Watch fixtures must register cancellation and joining immediately after launch,
before any startup assertion can fail. Otherwise a failed setup can leave the
execution lease open during temporary-worktree removal and restore process env
around a still-running watcher. Allow native helper/debugger startup separately
from assertions about cancellation latency or output drainage. Readiness markers
that carry data must be read successfully once; reuse those bytes instead of
checking existence and reopening across a Windows sharing race.
...
Delve startup/stop regressions must distinguish a listening socket from an initialized debugger. Keep the portable fixture's initialization barrier and assert nonblocking state readiness, RPC cancellation, halt/kill-detach on explicit stop and cancellation, concurrent stop joining, a resume between halt and detach, and OS fallback when RPC stays unavailable. Real CI probes exercise immediate debugger readiness and a running debuggee that publishes its PID; assert that PID is gone after cleanup, in addition to cleared service status. Do not mask shutdown failures by extending test deadlines or removing the engine's liveness check.

For a task-log presentation check, emit stdout, stderr, then stdout again: all three lines should be prefix-free, only stderr should be red, and literal bracketed text should remain visible. Check blank lines and failure excerpts too; retained stdout must be plain and stderr marked `E: `. Producer checks must cover callback/process output, multiline and partial interactive stderr, prompt responsiveness, stream switches and unchanged event payloads. CLI JSONL and cursor pages expose compact retained text; GitHub replay must still neutralize workflow markers.
...
- Real Payload 3.88.0 / Next 15.4.11 / Postgres coverage runs with `DEVFLOW_E2E_PAYLOAD=1 go test ./examples/payloadcms-postgres -run 'TestPayload(TUI)?WorkflowE2E' -v -timeout 25m`. The engine workflow covers dev startup/watch edits, blocks and fields, rename/create SQL, data-preserving replay, unchanged restart, blank migration choices, headless/cancel, mixed-mode warning and destructive push decline/retry. The TUI workflow compiles the CLI, bootstraps a fresh project in a real terminal, installs dependencies, creates/seeds a Payload table, edits the collection module, creates migrations with `m`, answers both UP and DOWN rename menus, and confirms a later destructive warning exclusively with terminal keys. The later prompt verifies that migration authoring preserves the resumed watcher's interaction policy. Verify database values/schema, answered prompt identity, restarted readiness, and TUI-owned daemon/service shutdown. Both tests run in the native Linux Docker matrix; see [Payload workflow verification](payload-workflow-verification.md).
- Portable terminal fixtures cover repeated multi-option menus with coalesced cursor visibility, positioned rows, compressed spaces, selection redraws, fragmented UTF-8/ANSI, asynchronous logger suffixes, actual keystroke forwarding, declined/failed prompt cleanup and no retries after decline. Replay captured OSC titles inside words; cover BEL/ST terminators, partial strings, preserved choices and prompt cancellation. Keep synthetic rendering regressions distinct from captured native output; retain escaped output on failure for Windows diagnosis. CLI/store/TUI tests cover choice zero, typed/range validation, reconnect, arrow/Enter/Escape, plus virtual-time readiness pause, attempt isolation and operation deadlines.
...
- optional project-upgrade coverage for terminal-only prompting before installation, affirmative/declined/EOF/canceled input, explicit `--project` and `--project=false`, noninteractive JSON, worktree selection, installed-binary version evidence rather than a second latest lookup, retained global success on project failure, staged module updates preserving application dependencies, and active/newly activated Devflow replacement rejection
...
- project-version startup coverage for root-module requirements, versioned/local replacements, absent pins, explicit source override and ignored `go.work`; require early handoff of unknown commands/flags, coherent launcher/effective version evidence, selected bundled docs, adapter-independent recovery, checksum failures without tracked module writes, cache reuse/invalidation and cancellable serialized preparation
...
- terminal-backed migration confirmation: real stdin terminal detection on every platform, warning retention before asking, accept/decline/handler-rejection/cancellation/no-handler outcomes, process-tree cleanup, large/final partial output and secret-echo suppression. Replay captured ConPTY controls across chunk boundaries, forbid callbacks after prompt failure, and verify cancellation cannot report success after cleanup. Failed TUI actions must leave task logs visible. See [Prisma confirmation verification](prisma-confirmation-verification.md)
...
- real-watch prerequisite recovery with converging branches and separate schema/migration inputs: barriers plus `synctest.Wait` force a migration-needed failure before sibling generation starts, then migration-only repair must execute canceled prerequisites and groups before consumers in the same run. Verify retained states, event ordering, flush success, recursive recovery, continued failure without sync-only retries, cached/healthy prerequisite reuse, manual service stops, restart/warmup barriers and preservation of completed historical attempts when a later cycle cancels their tasks
...
Exit observations must use stock tcell/tview APIs. Verify decoded controls versus handler decisions, unchanged forwarded events, first-failure preservation and loop-return/cleanup order without recording printable input. Native Windows subprocess tests verify the parent's actual numeric/hex exit observation independently of child output. Daemon tests hold the transition lock to prove the caller/request record is durable before cleanup, with success and failure outcomes correlated. Existing log inspection must remain read-only and keep its JSONL contract. See [focused diagnostic verification](exit-diagnostics-verification.md) for scope and missing upstream boundaries.
