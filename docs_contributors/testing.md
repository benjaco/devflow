...
- CLI JSON output shape, including command-level lifecycle coverage for `run`, CI stderr progress with stdout-only final failure diagnostics and bounded early-marker excerpts, `status`, `logs`, `instances`, target-scoped required-env doctor/strict exits, cache key/path/manifest handoff, and `stop`
