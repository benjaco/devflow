...
`CommandSpec.Terminal` gives a child an owned PTY/ConPTY. `ParsePrompt` lets adapters recognize dynamic menus from at most 64 KiB of output after a 100 ms quiet period and encode exact input bytes. Literal prompt specs still work. Timer generations and reader completion prevent stale callbacks; secret suppression applies to both paths. This is structured interaction with declared tools, not a general terminal attachment.

Select prompts retain ordered `choices`; responses carry one zero-based `choice` index. `cancel` stops the interaction with `interaction_cancelled`. Service startup pauses its readiness budget only while that attempt has a pending operator question, and cannot commit `AfterReady` during that wait. Flush reports the waiting service as unready. Other attempts, operation deadlines and prompt expiry remain independent.
...
PayloadCMS development push and migration replay are separate database workflows. A push-mode `up`/watch service uses `ConfigureDevService`; it must not depend on `payload.Migrations(b)` against that same database. Use migrations against a separate database, or disable push entirely and adopt a migrations-only workflow. Migration creation remains an explicit action registered by `payload.NewMigration(b)`, using Payload snapshots without requiring a live database. Both migration commands set `PAYLOAD_CONFIG_PATH` from the component config.

Payload's adapter enables owned terminal execution for migration apply/create and configured development commands. Its parser recognizes the actual Payload 3 confirmation rendering and Drizzle/Hanji create/rename menus for columns, block tables, enums and other named entities. The generic prompt API/TUI transports choices; Payload text recognition and arrow/Return encoding stay in `pkg/database`. Menus can repeat for both UP and DOWN generation. Confirmation messages preserve warning details; a decline fails immediately because Payload can otherwise exit zero and trigger a new-file convergence retry. Custom literal prompt declarations remain available. Default schema inputs include `src/collections`, `src/globals`, `src/fields`, and `src/blocks`; adapters must declare additional config/plugin paths. Migration creation requires a new file, preserves existing history and retries only successful missing-output attempts.

Payload prompt recognition must tolerate ConPTY coalescing cursor visibility between questions. Before reading visible text or cursor controls, the adapter removes complete OSC strings (including terminal titles) and waits for any unfinished OSC terminator; titles can interrupt a question word and their contents are opaque. The adapter recognizes the latest complete question, translates positioned rows and forward spacing into text separators, and ignores completed output and selection redraws away from Hanji's initial create option. This relies on the adapter sending only Down/Return for a selection; it is not arbitrary terminal navigation. Default literal confirmations end at stable punctuation, without requiring a trailing space. The generic process reader remains unchanged.

`PayloadCMSComponent.ConfigureDevService` also ensures its managed database, adds narrow schema/package inputs, and computes a password-free database/schema fingerprint in `BeforeRun`. It sets `PAYLOAD_SCHEMA_PUSH` in the task-local env. `AfterReady` promotes the pending fingerprint only after readiness succeeds; a declined/failed push leaves the prior applied key intact. Readiness must establish that Payload initialized its database, not merely that Next opened a TCP port. The Payload config consumes `push: process.env.PAYLOAD_SCHEMA_PUSH === 'true'`. `TaskBuilder.ConfigureCommand` decorates declared builder commands independent of call order; custom `Run` callbacks must declare their own process interaction options.
...
Restore validates cached artifacts and stages complete copies under a unique `.devflow/cache-restore-*` directory before replacing any output. Duplicate and descendant declarations normalize to the containing output. Publication retains existing outputs as backups and rolls back completed replacements if a later replacement fails. If rollback itself fails, the returned error identifies the retained recovery directory. Cancellation and damaged entries leave existing outputs intact before publication; ordinary cache corruption is a miss, while filesystem or cancellation errors propagate to the engine instead of silently executing the task.

`internal/fsutil.MovePathWritable` accepts the execution context and retries only the rename operation on Windows access/sharing/lock conflicts, using the existing state-file replacement policy: at most two seconds of retry waiting with exponential delays capped at 50 ms. Destination removal and permission preparation happen once. Backup, install and rollback use this path; validation artifact transfers pass their own cancellation context too. Other platforms keep their single rename attempt. A returned move failure preserves the native cause and reports paths, retry count and elapsed time; access denied alone does not identify a lock holder.

Cancellation prevents further publication but cannot prevent recovery of earlier outputs. Rollback uses a context independent of caller cancellation and tries every affected output; each rename has the same two-second bound, so total recovery time depends on the number of outputs. A completed final installation remains the successful publication boundary. Recovery failures retain backups and put the recovery directory first in the error, before details that bounded CLI diagnostics may truncate. Initial destination removal can also fail; it is not repeatedly deleted as part of rename retry.

The engine writes a redacted, bounded restore failure to the attempt log and normal log event before completion, even if the task callback never executed. Copy progress is not proof of a committed cache hit. This change preserves task-state classification, service shutdown ordering and immutable completed-attempt evidence; it does not add a public restore-error API. See [cache-restore verification](cache-restore-verification.md).
...
Exit observations reuse these logs and the existing Windows bootstrap parent. The TUI observes decoded controls through tview's input capture and backend identity through tcell's public API; neither library's input implementation is copied. The Windows parent records its child's native status independently, and an existing daemon request ID links owned TUI cleanup to caller/stop records in `daemon.log`. Raw console records, decoder internals and failures hidden by the dependency remain unavailable; deeper observation belongs behind an upstream-supported hook. There is no separate diagnostic session store or automatic inference of unexplained termination.
...
- prompt popups for interactive confirm, text and select questions emitted by daemon-owned work
...
- in normal mode the middle workspace is responsive: terminals at least 120 columns wide and 24 rows high place the stable task selector on the left and logs on the right, while compact terminals retain the vertical task-over-log layout
- attention mode places a compact instance box beside the state/name task list in a 4–7-row strip above borderless, full-width logs; target/mode lead the instance details, while paths, DB details, URLs and aggregate counts are hidden. Logs remain visible down to 40×10. The footer retains source/follow/focus context and action status. Reparent existing widgets without changing focus or scroll ownership
- task-log rendering hides the retained `E: ` stderr marker and colors that line red, escaping child text before adding TUI markup. Stdout stays plain. Apply the same presentation to failure excerpts; preserve raw snapshots and one rendered row per retained line for scrolling. Daemon logs retain their text
