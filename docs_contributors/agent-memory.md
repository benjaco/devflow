...
- `run --ci --json` keeps stdout machine-clean with one final result and streams state/cache/log progress to stderr. Failures must be self-contained enough for first diagnosis: top-level error, failed-node log path, bounded log tail, every selected node's final run snapshot/duration, and cache hit/miss timings. Register the event subscription synchronously before starting the engine; subscribing inside the progress goroutine can miss `run_started` for fast targets.
...
- Keep task cache storage in one OS user cache folder (`<os.UserCacheDir()>/devflow/cache`) and namespace entries by project. Per-worktree logs/state stay in the worktree `.devflow/`. CI integrations must use `cache path --json` and `cache key --target ... --json` rather than duplicating OS path or fingerprint logic.
