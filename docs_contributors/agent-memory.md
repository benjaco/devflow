...
- Prefer narrow, semantic fingerprints over hashing the whole repo. Use `Inputs.Filtered`/`project.Filtered(...)` when the normal graph inputs are right but only part of each file should affect a cache key. Keep this generic: filters live in `pkg/project`, watch matching remains path-based, and helpers such as `GoCommentLinesStartingWith("@")` plus `GoStructDeclarations()` should support Swagger-like cases without hardcoding Swaggo into core.
