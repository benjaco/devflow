...
- `run --ci --json` keeps stdout machine-clean with one final result and streams state/cache/log progress to stderr. Failures must be self-contained enough for first diagnosis: top-level error, failed-node log path, bounded log tail, every selected node's final run snapshot/duration, and cache hit/miss timings.
