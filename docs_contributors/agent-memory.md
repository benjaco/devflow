...
- CI installs Delve and runs real `dlv exec` smoke tests on Linux, macOS, and Windows, including a watch-mode source edit that must relaunch the debug service with a new PID, live debug listener, and intact attach metadata. The workflow uses Go 1.24 so the pinned Delve `v1.26.2` tool install is deterministic even though the module's own `go.mod` remains at Go 1.23 for now.
...
PayloadCMS/Postgres contract: normal `up`/watch targets apply existing Payload migrations through `payload.Migrations(b)` and should not create migrations implicitly. Migration creation uses the action registered by `payload.NewMigration(b)`, reads the action input `name` through `DEVFLOW_MIGRATION_NAME`, and can surface Payload confirmation prompts for destructive changes through the generic interactive prompt path. Prompt specs support alternate `Patterns` and `Repeat` for tools that ask multiple similar confirmation questions.
